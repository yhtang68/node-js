...
export function generateReview(jobs: JobEmail[]) {
  ...
  const htmlOutputPath = path.join(outputDir, 'Review-Linked-In-Jobs.html');
  const jsonOutputPath = path.join(outputDir, 'Review-Linked-In-Jobs.json');
  const jsonPayload = {
    summary: {
  ...
  console.log(`Review-Linked-In-Jobs.html generated at ${htmlOutputPath}`);
  console.log(`Review-Linked-In-Jobs.json generated at ${jsonOutputPath}`);
}
